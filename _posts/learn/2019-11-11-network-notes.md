...
- （4.4.1）**IP 数据报分片（IP datagram fragmentation）**
  - 最大传输单元（maximun transmission unit，**MTU**）：链路层帧的最大数据长度
  - 不同的链路（link）可以拥有不同的网络层协议，因此对应不同的 MTU，如 $MTU_{Ethernet} = 1500 bytes，MTU_{WAN} \leq 576bytes$
  - 分片重组（reassemble）由终端完成
  - （🌰 Figure 4.14）IP 数据报中与分片有关的头部字段：标识号（*identification number*）、信号（*flags*）、分片偏移（*fragmentation offset*）
    - 标识号：同一个数据报的不同分片拥有相同的标识号
    - 信号：$flag = 1$ 标志当前是最后一个分片，其它分片 $flag = 0$（解决丢失问题）
    - 分片偏移：用于目标主机重组分片时确定分片在原数据报中的位置
